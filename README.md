...
- **Per-engine overrides:** the Track Map is shared by every engine, which is what you want for the track's identity and offset. When one engine drives lights and another drives video, what differs is the triggers and the cue points. The editor's **Scope** selector switches from "Global" to "Engine N overrides": an override entry replaces, for that engine only, the triggers, the BPM multiplier and the cue points of the global entry, while offset and notes stay global (shown read-only for context). **Copy from global** starts an override from the global entry so you change only what differs; a Learn or Add in override scope also starts from the global triggers when the track has a global entry. An override with no triggers means "this engine does nothing on this track". Engines without overrides behave exactly as before; overrides are stored with the engine's settings and travel in the backup bundle
...
- **One audio format** — the playback device uses the global SAMPLE RATE and BUFFER SIZE like every other audio device STC opens, in either direction and across all engines; on an interface shared with the LTC output they are, in fact, the same device. Files at another sample rate are resampled on the fly.
...
- **Manual value** — an 8-digit hexadecimal value you enter. The last digit goes into binary group 1 and the first into group 8, the order the standards use when they assign digits to groups and the one most readers print (libltc-based tools, Sidus TC Sync, Pico-Timecode), so what you type is what they show. SMPTE ST 12 sets no order for free-form hex and some readers (GoPro, and STC before this version) print the groups the other way round: if yours shows your value reversed, turn on **REVERSE DIGIT ORDER** under the field. The date and name modes are addressed by group number as the standards specify, so they read the same everywhere
...
- **Debug: buffer counter** — every group carries the low four bits of the audio callback count, so an LTC capture shows which audio buffer each frame was packed in and where the buffer boundaries fall. For chasing an interface that drops or delays audio (see the gap counter above); not for shows
